...
public interface DisponibilidadeRepository extends JpaRepository<Disponibilidade, Long> {
        ...
        List<Disponibilidade> findByMentorAndDataHoraInicioAfterAndDisponivelTrueOrderByDataHoraInicio(User mentor,
                        LocalDateTime agora);
        ...
        @Query("SELECT COUNT(d) > 0 FROM Disponibilidade d WHERE d.mentor.id = :mentorId " +
                        "AND d.dataHoraInicio <= :inicio AND d.dataHoraFim >= :fim AND d.disponivel = true")
        boolean existsByMentorIdAndHorario(Long mentorId, LocalDateTime inicio, LocalDateTime fim);
